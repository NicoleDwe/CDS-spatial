---
title: "Spatial Statistics Introduction exercises"
...
Aha! Forcing the aspect ratio to 1 should prevent distortion of areas.
...
```
Dazzling disc drawing! The `disc()` function in `spatstat` conveniently lets you plot a disc.

# Task 4: Quadrat count test for uniformity 
...
# Task 5: Creating a uniform point pattern with spatstat
...
# TEST:  Point Pattern testing

Two `ppp` objects have been created called `p1` and `p2` and saved as .rds. Load them from thes data/ folder and test if they are clustered or regular, and select the right answer. You'll need to run `quadrat.test` four times, once for each process with each hypothesis.
...
# testing p1 
## regular
quadrat.test(p1, alternative = "regular") # nope
## clustered
quadrat.test(p1, alternative = "clustered") # yes

# testing p2
## regular
quadrat.test(p2, alternative = "regular") # nope
## clustered
